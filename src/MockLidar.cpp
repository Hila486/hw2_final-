...
types::LidarConfigData MockLidar::config() const{ // return lidar capabilities
    return config_;
}

types::LidarScanResult MockLidar::scan(Orientation scan_orientation) const { // return array of beams, each beam is a LidarHit object
...
PhysicalLength MockLidar::traceBeam(const Orientation& beam_orientation) const { // For this one beam direction, how far until I hit an occupied voxel?
